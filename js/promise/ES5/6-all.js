/**
 * ES5实现Promise
 *  1. 实现all方法，输入一组Promise数组（也可能是可迭代对象），返回一个Promise
 *  2. Promise的状态取决于每一个Promise，全成功则成功，有一个失败则失败
 *  3. 成功，则value为所有Promise成功value组成的数组，失败则reason为失败的reason
 *  4. 要求成功时返回value的数组内部顺序和输入顺序一致
 */
